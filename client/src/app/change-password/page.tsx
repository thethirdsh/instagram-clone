'use client'

import Footer from '@/components/footer'

const ChangePassword = () => {
  return (
    <div className="flex flex-col items-center bg-white w-full h-full">
      <div className="flex flex-col justify-center items-center py-20 lg:py-40">
        <div className="flex flex-col items-center rounded border-[1px] border-gray-300 w-[335px] h-[435px]">
          <p className="font-semibold text-center text-xl pt-9">
            Change your Password
          </p>
          <p className="text-xs text-gray-500 text-center pt-8 w-[80%]">
            Your password should be 10-32 characters long and include uppercase
            letters, lowercase letters, numbers, and special characters ($, @,
            #, !).
          </p>
          <form
            action="submit"
            className="flex flex-col items-center pt-6 w-full"
          >
            <input
              type="password"
              placeholder="Current password"
              className="border-[1px] border-gray-300 rounded w-[80%] text-black text-xs font-light bg-[#FAFAFA] pl-2.5 mt-1.5 h-10 focus:outline-gray-500"
            />
            <input
              type="password"
              placeholder="New password"
              className="border-[1px] border-gray-300 rounded w-[80%] text-black text-xs font-light bg-[#FAFAFA] pl-2.5 mt-1.5 h-10 focus:outline-gray-500"
            />
            <input
              type="password"
              placeholder="Confirm new password"
              className="border-[1px] border-gray-300 rounded w-[80%] text-black text-xs font-light bg-[#FAFAFA] pl-2.5 mt-1.5 h-10 focus:outline-gray-500"
            />
            <button
              type="submit"
              className="flex justify-center items-center bg-[#0099FF] rounded-lg mt-11 w-[80%] mt-4 h-8"
            >
              <p className="text-white text-sm font-medium">Change password</p>
            </button>
          </form>
        </div>
        <div className="hidden lg:flex mt-32">
          <Footer />
        </div>
      </div>
    </div>
  )
}

export default ChangePassword
